    ...
    for i, (x, y, title, body, bg_col) in enumerate(chain):
        ...
        if bg_col == '#3a0000':
            rect2 = mpatches.FancyBboxPatch((0.02, y - 0.06), 0.96, 0.07,
                                             boxstyle="round,pad=0.005", facecolor='#3a0000', edgecolor=RED, linewidth=2)
            ax.add_patch(rect2)
        ax.text(0.05, y - 0.01, title, color=ACCENT if i in [0] else RED if bg_col == RED or bg_col == '#3a0000' else YELLOW,
                fontsize=8, fontweight='bold', va='top')
        ax.text(0.40, y - 0.01, body, color=TEXT if bg_col != '#3a0000' else RED,
...
def page_fta(pdf):
    ...
    node(ax, 0.5, 0.88, 0.55, 0.055,
         'РАЗРУШЕНИЕ КЛАПАННОГО УЗЛА ГБЦ\nQSK50 MCRS — NTE200 (24 машины)', '#3a0000', RED, 8.5, RED)
    ...
    for cx, cy, text, color in root_causes:
        node(ax, cx, cy, 0.30, 0.085, text, '#1a2a20' if color==ACCENT else PANEL, color, 7.5, color)
...
def page_evidence(pdf):
    ...
    for i, row in enumerate(rows):
        ...
        bg = '#1a2a20' if is_proven else '#2a1a1a' if is_refuted else (PANEL if i % 2 == 0 else BG)
...
def page_conclusions(pdf):
    ...
    for code, color, title, text in conclusions:
        rect = mpatches.FancyBboxPatch((-0.01, y - 0.095), 1.02, 0.10,
                                        ...
                                        facecolor='#1a2a20' if color==ACCENT else PANEL, edgecolor='none')
        ax.add_patch(rect)
...
if __name__ == '__main__':
    output = '/home/user/NTE200/Анализ_коренных_причин_ГБЦ_QSK50_NTE200_p3.pdf'
