...
def page_ccec(pdf):
    ...
    rect_c = mpatches.FancyBboxPatch((-0.01, 0.44), 1.02, 0.075,
                                      boxstyle="round,pad=0.01", facecolor='#2a1a1a', edgecolor=YELLOW, linewidth=1.5)
    ax.add_patch(rect_c)
    ax.text(0.01, 0.511,
...
def page_ecm_table(pdf):
    ...
    for i, row in enumerate(rows):
        ...
        bg = '#2a1a1a' if is_crit else (PANEL if i % 2 == 0 else BG)
...
def page_tib(pdf):
    ...
    steps = [
        ...
        (0.53, 0.56, 0.42, 0.045, 'NTE200: TIB_max = 200% нормы. Доза УДВОЕНА.', '#2a1a1a', RED),
        (0.05, 0.49, 0.90, 0.045, 'Избыточная доза топлива → неполное сгорание → рост EGT (+100-150°C)', '#2a1a1a', RED),
        (0.05, 0.42, 0.90, 0.045, 'Температура в камере сгорания циклически превышает расчётную', '#2a1a1a', RED),
        (0.05, 0.35, 0.90, 0.045, 'Тепловой поток на тарелку клапана (сторона газов): >700°C локально', '#2a1a1a', RED),
        (0.05, 0.28, 0.90, 0.045, 'Начало ползучести Inconel 751 при T > 650°C (CES51005, п. 4.3)', '#2a1a1a', RED),
        (0.05, 0.21, 0.90, 0.045, 'Пластическая деформация тарелки → нарушение прилегания к седлу → утечка газов', '#2a1a1a', RED),
        (0.05, 0.14, 0.90, 0.045, 'ОТКАЗ: прогар клапана, газовый удар, разрушение ГБЦ', '#3a0000', RED),
    ]
...
if __name__ == '__main__':
    output = '/home/user/NTE200/Анализ_коренных_причин_ГБЦ_QSK50_NTE200_p2.pdf'
