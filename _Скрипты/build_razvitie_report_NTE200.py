...
# 8c Тесты форсунок
s=slide(); header(s,"Тесты форсунок (стенд CR-2) и отказы","ТОПЛИВО · ФОРСУНКИ")
# #82 grid 16 injectors
verd={'1L':0,'1R':2,'2L':0,'2R':2,'3L':1,'3R':2,'4L':1,'4R':1,'5L':0,'5R':0,'6L':0,'6R':0,'7L':1,'7R':1,'8L':0,'8R':2}
txt(s,0.5,1.4,12,0.34,"#82 — испытаны все 16 форсунок (стенд «Поток CR-2», 03.06.2026, Cummins CRI 2867147):",12,True,INK)
cw=0.74; x0=0.5; y0=1.85
order=[f"{i}{s2}" for i in range(1,9) for s2 in ('L','R')]
for i,c in enumerate(order):
    v=verd[c]; col=GRN if v==0 else (ORG if v==1 else RED)
    cx=x0+(i%8)*(cw+0.06); cy=y0+(i//8)*(0.72)
    rect(s,cx,cy,cw,0.6,col); txt(s,cx,cy+0.06,cw,0.22,c,12,True,WHITE,align=PP_ALIGN.CENTER)
    txt(s,cx,cy+0.30,cw,0.22,('норма' if v==0 else f'брак {v}'),9,False,WHITE,align=PP_ALIGN.CENTER)
txt(s,0.5,3.45,12,0.3,"✅ норма 7 · ❌ брак по 1 точке 5 · ❌❌ брак по 2 точкам 4  →  дефектных 9 из 16 (56%)",12,True,RED)
rect(s,0.5,3.95,12.3,2.1,WHITE); rect(s,0.5,3.95,0.12,2.1,ORG)
txt(s,0.72,4.05,11.9,1.95,"Характер брака — ПЕРЕЛИВ на малой нагрузке/пилоте (точки P03/P04 выше допуска) → локальное переобогащение, вклад в рост EGT (дополнительно к TIB=200%).\n"
"Полевые отказы: #85 — износ плунжерной пары (8002 м/ч); #72 — утечка дренаж цил.5; #43 — замена форсунок. ЭБУ: #51 — FC 1549/323/1557 (цепи форсунок).\n"
"Драйвер износа — грязное топливо (хуже норм 18/16/13 и 12/9/6) для MCRS (зазоры 2–5 мкм). 730E на стенде не испытаны — прямого сравнения нет.",12,False,INK)
footer(s)
