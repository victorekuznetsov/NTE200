...
# 15 параметров ЭБУ
ECM15=[('C_TIB_Fuel_Adjust_Upper (учёт расхода, FCR)','100%','200%','%','учёт расхода топлива (Trip Information Base) — НЕ влияет на впрыск',0),
...
('Топливный код (карта впрыска)','FC0BU52','0WH95','—','иная карта давления/фаз/дымового ограничителя',2),
...
# ── чистота топлива ISO 4406 (ИСПРАВЛЕННЫЙ анализ) ──
# Источник: «Проверка топлива в Дорингтест июнь 2026.docx» — по 3 измерения на пробу, представит. = среднее кода.
# 3 точки: бак (как есть) / после грубой / после тонкой (= вход в ТНВД/форсунки). Коды по >4 / >6 / >14 мкм.
FUEL={
'46':{'бак':(18,16,15),'груб':(20,17,16),'тонк':(17,13,13)},
'47':{'бак':(18,17,15),'груб':(21,19,18),'тонк':(19,16,14)},
'81':{'бак':(20,16,14),'груб':(21,19,17),'тонк':(21,18,16)},
'82':{'бак':(21,16,15),'груб':(23,20,19),'тонк':(21,15,14)},
}
fmach=['46','47','81','82']
# верхние границы числа частиц/мл по коду ISO 4406 (Cummins: 18→2500, 16→640, 13→80)
RNG={12:40,13:80,14:160,15:320,16:640,17:1300,18:2500,19:5000,20:10000,21:20000,22:40000,23:80000}
# ГРАФИК 1: прогрессия чистоты по >4 мкм по стадиям фильтрации + эталоны
f,ax=fig(7.2,3.4)
x=np.arange(len(fmach)); w=0.26
bak=[FUEL[m]['бак'][0] for m in fmach]; gru=[FUEL[m]['груб'][0] for m in fmach]; ton=[FUEL[m]['тонк'][0] for m in fmach]
ax.bar(x-w,bak,w,color=HX(GRY),label='бак (как есть)')
ax.bar(x,gru,w,color=HX(ORG),label='после грубой')
ax.bar(x+w,ton,w,color=HX(BLU),label='после тонкой (вход ТНВД)')
ax.axhline(18,color=HX(RED),ls='--',lw=1.5,label='Cummins макс. в баке = 18')
ax.axhline(12,color=HX(GRN),ls='--',lw=1.5,label='цель у форсунок = 12')
for i in range(len(fmach)):
    if gru[i]>bak[i]: ax.annotate('', xy=(i,gru[i]+0.3),xytext=(i-w+0.0,bak[i]+0.3),arrowprops=dict(arrowstyle='->',color=HX(RED),lw=1.2))
ax.set_xticks(x); ax.set_xticklabels([f'#{m}' for m in fmach]); ax.set_ylim(0,25)
ax.set_ylabel('код ISO по >4 мкм'); ax.set_title('Чистота топлива >4 мкм по стадиям фильтрации (грубый фильтр УХУДШАЕТ)',color=HX(INK),weight='bold',fontsize=10); ax.legend(fontsize=7,ncol=2)
CH['fuel']=save(f,'fuel.png')
# ГРАФИК 2: топливо у форсунок (после тонкой) по 3 размерам vs цель Fleetguard 12/9/6
f,ax=fig(7.2,3.4)
sizes=['>4 мкм','>6 мкм','>14 мкм']; tgt=[12,9,6]; cmax=[18,16,13]
xx=np.arange(3); w2=0.19
colm={'46':HX(GRN),'47':HX(BLU),'81':HX(ORG),'82':HX(RED)}
for i,m in enumerate(fmach):
    ax.bar(xx+(i-1.5)*w2,FUEL[m]['тонк'],w2,color=colm[m],label=f'#{m}')
for j,t in enumerate(tgt):
    ax.plot([j-0.42,j+0.42],[t,t],color=HX(GRN),lw=2.2)
    ax.plot([j-0.42,j+0.42],[cmax[j],cmax[j]],color=HX(RED),lw=1.3,ls='--')
ax.text(2.0,6.4,'цель Fleetguard 12/9/6',color=HX(GRN),fontsize=8)
ax.text(2.0,13.4,'макс. Cummins 18/16/13',color=HX(RED),fontsize=8)
ax.set_xticks(xx); ax.set_xticklabels(sizes); ax.set_ylim(0,24)
ax.set_ylabel('код ISO 4406'); ax.set_title('Топливо у форсунок (после тонкой) — ни одна цель не достигнута',color=HX(INK),weight='bold',fontsize=10); ax.legend(fontsize=8,ncol=4)
CH['fuel2']=save(f,'fuel2.png')
# ГРАФИК 3: фактическое число частиц >4 мкм у форсунок (log) — масштаб превышения
f,ax=fig(7.2,3.0)
ton4=[RNG[FUEL[m]['тонк'][0]] for m in fmach]
cols=[HX(RED) if v>2500 else HX(ORG) for v in ton4]
ax.bar([f'#{m}' for m in fmach],ton4,color=cols)
for i,v in enumerate(ton4): ax.text(i,v*1.15,f'{v:,}'.replace(',',' '),ha='center',color=HX(INK),fontsize=9)
ax.axhline(40,color=HX(GRN),ls='--',label='цель 12 (≤40 ч/мл)')
ax.axhline(2500,color=HX(RED),ls='--',label='Cummins 18 (≤2500 ч/мл)')
ax.set_yscale('log'); ax.set_ylabel('частиц >4 мкм /мл (log)'); ax.set_ylim(20,60000)
ax.set_title('Фактическое число частиц >4 мкм у форсунок (≈32–512× над целью)',color=HX(INK),weight='bold',fontsize=10); ax.legend(fontsize=8)
CH['fuelcnt']=save(f,'fuelcnt.png')
...
def exec_summary():
    ...
    panel(s,0.5,2.55,12.33,1.4,"Отказы ГБЦ — МНОГОФАКТОРНЫЕ. ИНИЦИИРУЮТ: пыль/негерметичность впуска (засорённые фильтры, коррозия ОНВ) + загрязнённое топливо → износ направляющих/стержней клапанов и разброс форсунок. УСИЛИВАЕТ: калибровка ЭБУ (иной топл. код 0WH95, отключённые тепловые защиты, повышенные обороты) → перегрев EGT 567°C → выгорание клапана. СОПУТСТВУЕТ: нет Stellite.",RED,12,"Главный вывод (многофакторный)")
    rows=[["Фактор","Роль","Основание"],["Пыль / негерметичность впуска","инициирующий","фильтры #69, ОНВ/ОЖ ✓ (Si низкий ⚠)"],
    ["Загрязнение топлива → форсунки","инициирующий","ISO плохой, разброс P04 ✓"],["Калибровка ЭБУ (топл. код + защиты + обороты)","УСИЛИВАЮЩИЙ","~15 параметров, EGT 567 ✓"],
...
# реестр 47 машин — компактно на 1 слайде (2 колонки)
regs=sorted(REG,key=lambda z:int(z['gar']) if str(z['gar']).isdigit() else 999)
s=content("Реестр NTE200 — 47 машин","РЕЕСТР")
rows=[["Гар.№","ДВС №","Год"]]+[[x['gar'],x['eng_no'] or '—',x['year']] for x in regs]
h2=(len(rows)+1)//2
table(s,0.5,1.4,6.2,[0.25,0.5,0.25],rows[:1]+rows[1:h2],fsize=9,rowh=0.235)
table(s,6.9,1.4,6.2,[0.25,0.5,0.25],rows[:1]+rows[h2:],fsize=9,rowh=0.235)
txt(s,0.5,6.85,12.3,0.3,"47 машин NTE 200 (гар. №43–89), 2023 г.в., Cummins QSK50, привод GE. Полный реестр (PIN, гарантия) — в Obsidian.",9,True,GRY)
...
chart_slide("Нагрузка: анализ по нормальному распределению","НАГРУЗКА · СТАТИСТИКА",CH['loadnorm'],"μ=99,75%; σ=13,7; асимметрия −3,6; P95=110%, P99=113%, доля >120% ≈ 0,01%. Загрузка дисциплинирована у номинала → систематическая перегрузка НЕ является инициатором отказов.")
...
# помашинное масло ДВС — только агрегаты (полная помашинная таблица в Obsidian)
...
rows=[["Группа","NTE200 vs 730E"],["Топливный код (впрыск)","0WH95 vs FC0BU52"],["Защиты перегрева","все ОТКЛ vs Вкл"],["Обороты/регулятор","2100/20% vs 1990/0%"],["Горячий стоп","ниже пороги"]]
table(s,0.5,3.0,12.33,[0.35,0.65],rows,fsize=12,rowh=0.5)
# 15 параметров — 2 компактных табличных слайда (вместо 15 отдельных)
sev_lbl={2:'КРИТ',1:'важно',0:'фон'}
for ci in range(0,len(ECM15),8):
    s=content(f"15 параметров ЭБУ ({ci+1}–{min(ci+8,len(ECM15))})","ECM · ПАРАМЕТРЫ")
    rows=[["#","Параметр","730E","NTE200","Влияние на отказ","Риск"]]
    for idx in range(ci,min(ci+8,len(ECM15))):
        nm,a,b,u,eff,crit=ECM15[idx]
        rows.append([str(idx+1),nm,f"{a}{(' '+u) if u and u!='%' else ('%' if u=='%' else '')}",f"{b}{(' '+u) if u and u!='%' else ('%' if u=='%' else '')}",eff,sev_lbl[crit]])
    table(s,0.4,1.45,12.5,[0.04,0.26,0.13,0.13,0.36,0.08],rows,fsize=10.5,rowh=0.56)
...
s=content("Коды ECM (Fault Codes) — весь перечень","ECM · КОДЫ")
rows=[["FC","Машин","Lamp","Описание"]]+[[a,b,c,d] for a,b,c,d in FAULTS]
table(s,0.5,1.45,12.33,[0.12,0.15,0.15,0.58],rows,fsize=11,rowh=min(0.46,4.7/len(rows)))
...
panel(s,0.5,2.85,12.33,2.6,"Банк B (чётные цилиндры) систематически горячее банка A на ~60°C — признак межбанкового дисбаланса (геометрия наддува/возврата, группировка дефектных форсунок). Совпадает с зоной самых дефектных форсунок (B-банк 6/8) и самых горячих цилиндров (Ц2, Ц6).",RED,13,"Асимметрия")
...
for i in range(0,len(crit),4):
    s=content("Критичные форсунки — детально","ФОРСУНКИ · АЛЕРТЫ")
    for j,a in enumerate(crit[i:i+4]):
        yy=1.4+j*1.35
        rect(s,0.5,yy,12.33,1.2,WHITE); rect(s,0.5,yy,0.1,1.2,RED)
        txt(s,0.7,yy+0.06,4.0,0.3,f"{a.get('m','')} · {a.get('id','')}",13,True,INK)
        txt(s,0.7,yy+0.38,12.0,0.8,a.get('reason','')[:180],11,False,INK)
        txt(s,8.7,yy+0.06,4.0,0.3,a.get('type','')[:30],10,True,RED)
# прочие отклонения — компактной таблицей на 1 слайде
if others:
    s=content("Прочие отклонения форсунок (сводно)","ФОРСУНКИ · АЛЕРТЫ")
    rows=[["Машина · поз","Тип","Причина"]]
    for a in others: rows.append([f"{a.get('m','')} · {a.get('id','')}",a.get('type','')[:18],a.get('reason','')[:70]])
    table(s,0.5,1.45,12.33,[0.2,0.22,0.58],rows,fsize=11,rowh=0.5)
# 9.6 таблицы по машинам (полные значения): #73+#78 на одном слайде, #82 отдельно
def inj_rows(m):
    rows=[["Поз","Дата","P01","P02","P03","P04","Ст."]]
    for x in bym.get(m,[]):
        st={'ok':'✓','warn':'⚠','crit':'✗'}[sev_of(x)]
        rows.append([x['id'],x.get('date',''),x.get('p01','—'),x.get('p02','—'),x.get('p03','—'),x.get('p04','—'),st])
    return rows
s=content("Форсунки #73 и #78 — значения стенда","ФОРСУНКИ · #73/#78")
txt(s,0.5,1.3,5,0.3,"#73 (R1 критич.)",12,True,INK); txt(s,6.95,1.3,5,0.3,"#78 (3 перелива)",12,True,INK)
table(s,0.5,1.65,6.2,[0.13,0.22,0.16,0.16,0.13,0.13,0.07],inj_rows('#73'),fsize=10.5,rowh=0.42)
table(s,6.95,1.65,6.2,[0.13,0.22,0.16,0.16,0.13,0.13,0.07],inj_rows('#78'),fsize=10.5,rowh=0.42)
txt(s,0.5,6.6,12.3,0.3,"Допуски: P01 441–495 · P02 220–260 · P03 16–30 · P04 41–51 см³.",10,True,GRY)
s=content("Форсунки #82 — полные значения стенда (все 16)","ФОРСУНКИ · #82")
rows=inj_rows('#82'); rows[0]=["Поз","Дата","P01 (441–495)","P02 (220–260)","P03 (16–30)","P04 (41–51)","Статус"]
table(s,0.5,1.45,12.33,[0.1,0.18,0.18,0.18,0.13,0.13,0.1],rows,fsize=11,rowh=max(0.32,min(0.5,4.6/len(rows))))
...
panel(s,0.5,4.6,12.33,1.4,"Общий признак: ПРОСАДКА/увеличение высоты выпускных клапанов (рецессия седла) + износ направляющих втулок/стержней → потеря/изменение теплового зазора. По #46 есть ЧИСЛОВЫЕ замеры зазоров и высоты клапанов (KAMSS) — см. след. слайд.",ORG,12,"Что выявлено")
# числовые данные зазоров #46 (фото KAMSS image11)
s=content("Зазоры и высота выпускных клапанов #46 — числовые замеры (KAMSS)","КЛАПАНЫ · ЧИСЛА")
imv=sorted(glob.glob('_Изображения/KAMSS обследование 3 единиц/*.jpeg'))
try: s.shapes.add_picture(imv[2] if len(imv)>2 else imv[0],Inches(0.5),Inches(1.5),width=Inches(12.3))
except: pass
panel(s,0.5,4.9,12.33,1.7,"Замеры по двигателю #46 (QSK-50 №33232853): зазор ВПУСКНЫХ клапанов в норме (0,014 во всех 16 цил.), а зазор ВЫПУСКНЫХ обнулён (0,000) на цилиндрах 3,6,7,10,12,14,15,16 — клапан НЕ садится в седло (рецессия). Высота от плоскости ГБЦ до тарелки выпускного клапана — в красной зоне (67,5–69 мм). Поражение сосредоточено на горячих цилиндрах → подтверждает рецессию седла/просадку клапана как физический механизм отказа.",RED,12,"Интерпретация (числа, не качественно)")
# вердикт корреляции
...
# 10.1 — методика и две системы отсчёта (исправление)
s=content("Чистота топлива ISO 4406 — методика и нормы","ТОПЛИВО · ЧИСТОТА · МЕТОД")
panel(s,0.5,1.45,12.33,1.5,"ISO 4406: класс чистоты — три числа х/х/х = коды числа частиц >4 / >6 / >14 мкм/мл. Шкала ЛОГАРИФМИЧЕСКАЯ: +1 к коду = ×2 частиц. Дорингтест (ЛАМ Полюс Магадан) измерил по 3 раза каждую пробу в 3 точках: бак (как есть), после грубой, после тонкой очистки на машинах #46/#47/#81/#82.",ACC,12,"Что и как измеряли")
rows=[["Точка отбора","Эталон","Код","Смысл"],
["Бак / как поставлено","Cummins, макс. в баке","18/16/13","выше — нужна доп. фильтрация ДО бака"],
["Вход в ТНВД/форсунки (после тонкой)","Fleetguard / Горная Евразия","12/9/6","цель для ресурса MCRS (зазоры 2–5 мкм, 2100 бар)"]]
table(s,0.5,3.15,12.33,[0.34,0.28,0.13,0.25],rows,fsize=12,rowh=0.6)
panel(s,0.5,5.3,12.33,1.0,"ИСПРАВЛЕНИЕ методики: бак сравниваем с 18/16/13 (как поставлено), а выход тонкого фильтра — с целью 12/9/6 (что реально идёт к форсункам). Прежде ошибочно судили выход тонкой по баковому пределу 18 и только по >4 мкм.",RED,12,"Корректная база сравнения")
# 10.2 — прогрессия по стадиям + аномалия грубого фильтра
s=content("Чистота по стадиям фильтрации (>4 мкм)","ТОПЛИВО · ЧИСТОТА · СТАДИИ")
s.shapes.add_picture(CH['fuel'],Inches(0.4),Inches(1.45),width=Inches(7.4))
rows=[["Маш","Бак","Груб.","Тонк."],["#46","18/16/15","20/17/16","17/13/13"],["#47","18/17/15","21/19/18","19/16/14"],["#81","20/16/14","21/19/17","21/18/16"],["#82","21/16/15","23/20/19","21/15/14"]]
table(s,8.1,1.5,4.7,[0.22,0.26,0.26,0.26],rows,fsize=11,rowh=0.5)
panel(s,8.1,4.05,4.7,2.15,"АНОМАЛИЯ: после ГРУБОГО фильтра чистота ХУЖЕ, чем в баке (>4 мкм код растёт у всех 4 машин, #82: 21→23 = ×4 частиц). Признак насыщения/обводнения/неверного обслуживания фильтра — он сам выделяет частицы.",RED,11,"Грубый фильтр ухудшает")
# 10.3 — у форсунок vs цель + масштаб + фото загрязнения
s=content("Топливо у форсунок vs цель 12/9/6","ТОПЛИВО · ЧИСТОТА · ФОРСУНКИ")
s.shapes.add_picture(CH['fuel2'],Inches(0.4),Inches(1.4),width=Inches(6.3))
s.shapes.add_picture(CH['fuelcnt'],Inches(0.4),Inches(4.0),width=Inches(6.3))
imf=sorted(glob.glob('_Изображения/Отстойники NHL/*.jpg'))
if imf:
    try: s.shapes.add_picture(imf[0],Inches(6.9),Inches(1.4),height=Inches(2.5))
    except: pass
rows=[["Маш","Тонк.","vs 12/9/6","×над целью >4мкм"],["#46","17/13/13","✗","~32×"],["#47","19/16/14","✗","~128×"],["#81","21/18/16","✗","~512×"],["#82","21/15/14","✗","~512×"]]
table(s,6.9,4.05,5.9,[0.18,0.3,0.22,0.3],rows,fsize=11,rowh=0.42)
panel(s,6.9,6.0,5.9,1.15,"Цель 12/9/6 не достигнута НИГДЕ: у форсунок частиц >4 мкм в 32–512× больше цели. Фото: осадок/вода в топливном отстойнике NHL — прямое свидетельство загрязнения.",ORG,10.5,"Вывод")
# 10.4 — итоговый вывод по топливу
s=content("Чистота топлива — итог и роль в отказе","ТОПЛИВО · ЧИСТОТА · ИТОГ")
rows=[["Тезис (исправлено)","Факт","Статус"],
["Топливо в баке соответствует 18/16/13","18/17/15…21/16/15 — выше по >14 (и >4 у #81/#82)","✗ не соответствует у всех 4"],
["Бортовая фильтрация даёт 12/9/6 у форсунок","лучшее #46 17/13/13; худшее #82 21/15/14","✗ не достигнуто нигде"],
["Грубый фильтр улучшает чистоту","у всех 4 машин после грубого ХУЖЕ, чем в баке","✗ ухудшает (ревизия)"],
["Топливо дифференцирует NTE200 vs 730E","топливо общее для обоих парков","✗ общий фактор"]]
table(s,0.5,1.45,12.33,[0.36,0.42,0.22],rows,fsize=11.5,rowh=0.62)
panel(s,0.5,5.0,12.33,1.4,"Роль топлива: общий для парков фактор — НЕ дифференцирует отказы NTE200. Но загрязнение в 32–512× выше цели угрожает топливной системе MCRS (зазоры форсунок 2–5 мкм) → износ плунжерных пар → разброс дозирования (перелив/недолив P03/P04) → дисбаланс сгорания и EGT. ОТЯГЧАЮЩИЙ фактор по форсункам.",RED,12,"Связь с отказом (объективно)")
...
panel(s,0.5,3.5,12.33,1.8,"Дефектация #46: числовые замеры зазоров и высоты выпускных клапанов (рецессия седла, зазор 0,000 на ряде цилиндров) + проверка герметичности (% утечки). Подтверждает просадку клапана и перегрев именно B-банка — согласуется с DML и кодами ECM.",ORG,13,"Дефектация клапанов")
# фото KAMSS
...
if len(imgs)>=2:
    ...
    try: s.shapes.add_picture(imgs[14] if len(imgs)>14 else imgs[1],Inches(6.8),Inches(1.7),width=Inches(6.0))
    except: pass
    txt(s,0.5,6.55,12.3,0.4,"Слева — INSITE коды EGT по цилиндрам (#46); справа — проверка герметичности цилиндров (% утечки): хуже на горячем B-банке.",11,True,INK)
...
["Масло","CI-4 15W-40","то же","нет"],["Топл. код (впрыск)","0WH95","FC0BU52","ДА"],["Защиты перегрева","ОТКЛ","Вкл","ДА"],["Отказы ГБЦ","~17","0","ДА"]]
table(s,0.5,1.5,12.33,[0.3,0.25,0.25,0.2],rows,fsize=12,rowh=0.5)
panel(s,0.5,5.4,12.33,1.0,"Единственные дифференцирующие факторы — в калибровке ЭБУ. При прочих равных 730E с топл. кодом FC0BU52 и включёнными защитами — 0 отказов ГБЦ.",GRN,13)
...
table(s,0.5,1.45,7.5,[0.42,0.3,0.28],rows,fsize=11,rowh=0.5)
imd=sorted(glob.glob('_Изображения/KAMSS обследование 3 единиц/*.jpeg'))
try: s.shapes.add_picture(imd[6] if len(imd)>6 else imd[0],Inches(8.2),Inches(1.45),width=Inches(4.6))
except: pass
txt(s,8.2,4.85,4.6,0.9,"Cummins: признаки всасывания пыли — «отпескоструенный» датчик и износ кромок лопаток компрессора турбины (контроль негерметичности впуска).",10.5,False,ORG)
...
panel(s,0.5,4.45,12.33,1.5,"Вывод: ни один фактор не объясняет отказ в одиночку. Это МНОГОФАКТОРНАЯ ЦЕПЬ: контаминация (пыль+топливо) инициирует износ направляющих/стержней/форсунок, а калибровка ЭБУ (отключённые защиты + повышенные обороты + иной топл. код) усиливает тепловую нагрузку и доводит клапан до выгорания.",RED,13,"Синтез")
# FMEA
...
"Почему высокая EGT? — Отключены тепловые защиты ЭБУ (derate по T масла/триггеры, задержки 43–51 с) + иной топл. код 0WH95 + разброс форсунок.","Почему разброс форсунок и износ направляющих? — Загрязнённое топливо и пыль (негерметичность впуска, засорённые фильтры).",
...
panel(s,7.2,1.5,5.6,4.4,"Качественная матрица связей: сильнее всего связаны EGT↔защиты ЭБУ↔перелив форсунок (термокластер). Si (пыль) слабо связан с EGT, но связан с чистотой топлива. Подтверждает: термический кластер (ЭБУ/форсунки) — главный усилитель; пыль — отдельный, более слабый по маслу канал.",ACC,12,"Чтение матрицы")
...
"УСИЛЕНИЕ (ЭБУ): отключённые защиты перегрева + повышенные обороты + иной топл. код → нагрузка не снижается → ↑ EGT, дисбаланс банков","ПЕРЕГРЕВ: EGT до 567°C (лимит 520); термоудар фаски клапана (стойкость снижена — нет Stellite)","ОТКАЗ: выгорание/просадка клапана и седла → потеря герметичности → разрушение ГБЦ"]
...
rows=[["Доказательство","Вес","Статус"],["Топл. код 0WH95 + защиты ОТКЛ","🔴 ключевое","✓ Calterm/письмо Cummins"],
...
# ── Часть 14б: Досье по отказавшим машинам (компактно) ──
s=slide(); section(s,"ПРИЛОЖЕНИЕ A","Досье по отказавшим машинам NTE200")
REPBY={}
for m,d,h,op in REPAIRS: REPBY.setdefault(m,[]).append(f"{d}: {op}")
regby={str(x['gar']):x for x in REG}
failm=sorted([m for m in REPBY if m.isdigit()], key=lambda z:int(z))
# сводная таблица по отказавшим машинам (вместо 18 отдельных слайдов)
for ci in range(0,len(failm),11):
    s=content(f"Сводное досье отказавших машин ({ci+1}–{min(ci+11,len(failm))})","ДОСЬЕ NTE200")
    rows=[["Маш","ДВС №","Статус ЭБУ","Fe/TBN","Нараб. ч","Ремонты ГБЦ"]]
    for m in failm[ci:ci+11]:
        r=regby.get(m,{}); ol=PM['oil'].get(m); ec=PM['ecm'].get(m); rp=REPBY.get(m,[])
        rows.append([m, (r.get('eng_no') or '—'), (ec['status'] if ec else '—'),
                     f"{ol['Fe'] if ol else '—'}/{ol['TBN'] if ol else '—'}",
                     (str(ec['hrs']) if ec else '—'), f"{len(rp)} соб."])
    table(s,0.5,1.45,12.33,[0.1,0.22,0.22,0.16,0.14,0.16],rows,fsize=11,rowh=0.46)
    txt(s,0.5,6.7,12.3,0.4,"Полные паспорта (PIN, гарантия, генератор) и помашинные хронологии — в Obsidian-реестре NTE200.",10,True,GRY)

# ── Приложение B: аналитические срезы ──
s=slide(); section(s,"ПРИЛОЖЕНИЕ B","Аналитические срезы и доп. данные")
# DML доп. параметры — сводно на 1 слайде
s=content("DML — прочие параметры (рампа/масло/наддув)","DML · ПАРАМЕТРЫ")
rows=[["Параметр","С нагрузкой","Норма","Статус"],
["Давление рампы CR","~1060 бар","до 2100 бар","✓ стабильно"],
["Давление масла","~380 кПа","≥241 кПа","✓ норма"],
["Давление наддува","~200 кПа","—","✓ норма"],
["Расход/режим","88% нагр., 1822 об/мин","—","высокая база EGT"]]
table(s,0.5,1.45,12.33,[0.3,0.27,0.23,0.2],rows,fsize=12.5,rowh=0.6)
panel(s,0.5,5.0,12.33,1.3,"Все базовые давления (рампа, масло, наддув) в норме → перегрев НЕ связан с механикой топливоподачи/наддува. Источник перегрева — калибровка (топл. код 0WH95, отключённые защиты, обороты) и разброс дозирования форсунок, а не давление в системе.",ACC,12.5,"Вывод")
# аналитические топы
...
# KAMSS доп фото — 1 слайд (4 фото в сетке)
imgs=sorted(glob.glob('_Изображения/KAMSS обследование 3 единиц/*.jpeg'))
if imgs:
    s=content("KAMSS — материалы обследования","KAMSS · ФОТО")
    pos=[(0.5,1.5),(6.8,1.5),(0.5,4.0),(6.8,4.0)]
    for k,(px,py) in enumerate(pos):
        if k<len(imgs):
            try: s.shapes.add_picture(imgs[k*2 if k*2<len(imgs) else k],Inches(px),Inches(py),width=Inches(5.9))
            except: pass
    txt(s,0.5,6.6,12.3,0.4,"INSITE-диагностика (коды EGT, защита двигателя) и дефектация клапанов/сёдел — обследование 3 единиц.",11,True,INK)

# ── Часть 15: Выводы ──
s=slide(); section(s,"ЧАСТЬ 15","Выводы и действия")
s=content("Неотложные действия","ДЕЙСТВИЯ · 🔴")
acts=["Перекалибровать ЭБУ NTE200 под профиль 730E: топл. код/карта впрыска как у 730E, включить защиты перегрева, убрать задержки derate, обороты 1990","Контрольный DML после перекалибровки — подтвердить снижение EGT","Заменить датчики EGT с FC1531/1619 (#82, #67)","Стендовая проверка/замена бракованных форсунок (перелив P03/P04)"]
