...
.company-logo-row{display:flex;align-items:center;gap:7px}
.company-logo-row svg{flex-shrink:0;filter:drop-shadow(0 0 6px rgba(62,240,175,0.45))}
.company-name{font-weight:800;font-size:16px;color:var(--acc);letter-spacing:3px;text-transform:uppercase}
...
.truck-del{background:none;border:none;color:rgba(255,64,96,0.35);cursor:pointer;font-size:13px;padding:2px 5px;line-height:1;border-radius:3px;flex-shrink:0;opacity:1;transition:all .15s}
.truck-del:hover{color:#ff4060 !important;background:rgba(255,64,96,0.15);transform:scale(1.2)}
...
<div class="hdr">
  ...
  <div class="hdr-center">
    <div class="company-logo-row">
      <svg width="32" height="32" viewBox="0 0 196 196" xmlns="http://www.w3.org/2000/svg" fill="none"><path d="M63.5941 165.078 114.786 64.5721 99.6758 31 83.5127 31 16 165.078 63.5941 165.078Z" fill="#3EF0AF"/><path d="M107.608 118.291 152.342 118.291 179.684 64.5721 164.574 31 119.559 31 134.669 64.5721 107.608 118.291Z" fill="#3EF0AF"/></svg>
...
function syncRangeToCharts(){
  ...
  panel.querySelectorAll('div[id^="ch-"]').forEach(div=>{
    if(div.id==='ch-dev')return;  // ch-dev is recomputed by renderCylDev, not relayouted
    try{
  ...
  // Recompute per-cylinder deviation bars for the selected time window
  if(currentTab==='cylinders')renderCylDev();
  setTimeout(function(){_syncing=false;},100);
}

// ═══ CHART RANGE SYNC (drag-to-zoom links all time-series charts on tab) ═════
function attachAllSync(){
  const panel=document.querySelector('.panel.show');
  if(!panel)return;
  panel.querySelectorAll('div[id^="ch-"]').forEach(div=>{
    if(typeof div.on!=='function')return;
    if(div.id==='ch-dev')return;  // categorical x-axis — not a time-series, excluded from sync
...
function pl(id,data,lay){
  ...
  // ch-dev uses categorical x-axis — never apply numeric time range to it
  if(id!=='ch-dev'&&activeRangeSec!==null){L.xaxis=L.xaxis||{};L.xaxis.range=[activeRangeStart,activeRangeSec];L.xaxis.autorange=false;}
...
function deleteTruck(key,e){
  e.stopPropagation();
  const d=TRUCKS[key];
  const label=d?d.model+' №'+d.truck:key;
  if(!confirm('Удалить '+label+' из дашборда?'))return;
...
function saveDashboard(){
  ...
  setTimeout(function(){
    try{
      ...
      // Normalize the save buttons before capturing. saveDashboard() sets the
      // HTML button to "⏳ Сохранение..." / disabled, and disabled + text are
      // reflected into outerHTML — so without this the saved file bakes in a
      // frozen, disabled button and can never be re-saved.
      const btnSnaps=[];
      btns.forEach(function(bb){
        if(bb.disabled||bb.textContent.indexOf('Сохранение')>-1){
          btnSnaps.push({b:bb,t:bb.textContent,d:bb.disabled});
          bb.textContent='⬇ Сохранить дашборд (HTML)';
          bb.disabled=false;
        }
      });
      ...
      // Restore the buttons' in-progress state (re-enabled at the end anyway)
      btnSnaps.forEach(function(s){s.b.textContent=s.t;s.b.disabled=s.d;});
...
// Recomputes ch-dev (per-cylinder deviation bars) for the current time window.
// Called by syncRangeToCharts so bars update when user zooms without re-rendering everything.
function renderCylDev(){
  if(compareMode||!activeTruck)return;
  const s=getSession(activeTruck);if(!s)return;
  const{ts,cyls}=s;
  const secs=timeSecs(ts);
  const aCyls=A_CYLS.filter(n=>cyls[String(n)]);
  const bCyls=B_CYLS.filter(n=>cyls[String(n)]);
  if(!aCyls.length&&!bCyls.length)return;
  // Indices that fall within the selected time window
  const idxs=(activeRangeSec===null)
    ?secs.map((_,i)=>i)
    :secs.reduce((acc,t,i)=>{if(t>=activeRangeStart&&t<=activeRangeSec)acc.push(i);return acc;},[]);
  if(!idxs.length)return;
  function cylMeanFilt(n){
    const arr=cyls[String(n)];if(!arr)return null;
    const vals=idxs.map(i=>arr[i]).filter(v=>v!=null&&!isNaN(v));
    return vals.length?vals.reduce((s,v)=>s+v,0)/vals.length:null;
  }
  const aMeans=aCyls.map(cylMeanFilt).filter(v=>v!=null);
  const bMeans=bCyls.map(cylMeanFilt).filter(v=>v!=null);
  const aAvgM=aMeans.length?aMeans.reduce((s,v)=>s+v,0)/aMeans.length:0;
  const bAvgM=bMeans.length?bMeans.reduce((s,v)=>s+v,0)/bMeans.length:0;
  const allC=[...aCyls,...bCyls].sort((a,b)=>a-b);
  const devs=allC.map(n=>{
    const v=cylMeanFilt(n)||0;
    const bk=A_CYLS.includes(n)?aAvgM:bAvgM;
    return{n,d:Math.round(v-bk),bank:A_CYLS.includes(n)?'A':'B'};
  });
  // Use Plotly.react directly to bypass pl()'s time-range axis logic
  const L=Object.assign({},PLY_BASE,{
    height:185,
    shapes:[mkLine(30,'rgba(255,184,48,.4)'),mkLine(-30,'rgba(255,184,48,.4)'),mkLine(60,'rgba(255,64,96,.4)'),mkLine(-60,'rgba(255,64,96,.4)')],
    yaxis:Object.assign({},PLY_BASE.yaxis,{title:'∆°C',zeroline:true,zerolinecolor:'rgba(255,255,255,.2)',zerolinewidth:1.5}),
    xaxis:Object.assign({},PLY_BASE.xaxis,{tickfont:{size:9}}),
    margin:{l:48,r:40,t:14,b:36}
  });
  Plotly.react('ch-dev',[{type:'bar',x:devs.map(d=>'Ц'+d.n),y:devs.map(d=>d.d),
    text:devs.map(d=>(d.d>=0?'+':'')+d.d+'°'),textposition:'outside',textfont:{size:9},
    marker:{color:devs.map(d=>{const ab=Math.abs(d.d),base=d.bank==='A'?'255,124,58':'91,143,255',op=ab>60?.95:ab>30?.7:.45;return`rgba(${base},${op})`;})},
  }],L,PLY_CFG);
}
...
function processINSITE(text,fname){
  ...
  for(let i=0;i<rows.length;i+=step){
    ...
    try{
      let dt;const dm=/^(\d{1,2})-([A-Za-z]{3})-(\d{4})$/.exec(d.trim());
      if(dm){const mo={Jan:1,Feb:2,Mar:3,Apr:4,May:5,Jun:6,Jul:7,Aug:8,Sep:9,Oct:10,Nov:11,Dec:12}[dm[2]];const[hh,mm,ss]=t.split(':').map(Number);if(mo)dt=new Date(+dm[3],mo-1,+dm[1],hh||0,mm||0,ss||0);}
      if(!dt||isNaN(dt))dt=new Date(d+' '+t);
      if(isNaN(dt))continue;
      out.ts.push(dt.toISOString().substring(0,19));
    }catch{continue;}
...
// ═══ INIT ════════════════════════════════════════════════════════════
// Truck list is pre-rendered as static HTML — do NOT call buildSidebar()
// here or it will clear the list before JS finishes rebuilding it.
function _init(){
