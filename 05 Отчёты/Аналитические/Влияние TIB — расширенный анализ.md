...
## Приложение. Публичные описания цилиндрового балансирования (справочно, НЕ про TIB)

> [!warning] Важно: это про другую функцию
> Раздел оставлен как **общий справочный материал** о поцилиндровом балансировании подачи (cylinder balancing / injector trim) в дизелях. **Это НЕ параметр TIB** из данной калибровки: в аутентичном экспорте `TIB = Trip Information Base` (учёт рейса/расхода), и `C_TIB_Fuel_Adjust_Upper_Limit` к балансировке впрыска отношения не имеет (см. §1–2). Балансировка подачи в QSK50 MCRS, если и применяется, управляется логикой топливного кода/впрыска, а не группой TIB. Совпадение в названиях («…Fuel_Adjust…») и породило первоначальную путаницу.
...
> Поцилиндровое балансирование (по крутящему моменту/вкладу цилиндра) — реальный, независимо документированный механизм дизельных ЭБУ. Но он **не тождествен параметру TIB** этой калибровки и в открытых источниках с аббревиатурой «TIB» не связан. Поэтому этот раздел — фон по смежной функции, а **не** обоснование влияния TIB на впрыск: значение `C_TIB_Fuel_Adjust_Upper_Limit` 200% vs 100% относится к учёту расхода топлива (FCR), см. §2.
