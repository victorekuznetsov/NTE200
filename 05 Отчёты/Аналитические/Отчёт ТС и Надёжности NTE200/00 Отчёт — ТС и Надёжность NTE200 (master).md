...
> Коренная причина — **системное снижение тепловой защиты в калибровке ЭБУ NTE200** относительно 730E-DC (~15 отличающихся параметров): **иной топливный код 0WH95** (vs FC0BU52 = иная карта впрыска), **отключённые триггеры перегрева** (масло/ОЖ/наддув/топливо), задержки снижения мощности 43–51 с и повышенные обороты (2100, до 2225 vs 1990 об/мин) с droop 20 %. Это подтверждено **аутентичным экспортом Calterm** и письмом Cummins.
...
> По аутентичному экспорту Calterm ([[Calibration_Compare_730E_vs_NTE200 (Calterm)]]) уточнено: **TIB = Trip Information Base** (учёт рейса/расхода), параметр `C_TIB_Fuel_Adjust_Upper_Limit` (100→200 %) относится к **учёту расхода топлива (FCR) и на впрыск не влияет**. Прежняя формулировка «удвоенная коррекция топлива TIB» снята; носитель отличий впрыска — топливный код `0WH95`.

| Фактор | Вердикт | Основание |
|---|---|---|
| Калибровка ЭБУ (топл. код 0WH95 + отключённые защиты + обороты) | 🔴 Коренная причина | [[Calibration_Compare_730E_vs_NTE200 (Calterm)]] · [[ecm_params_NTE200_vs_730E]] ✅ |
...
```mermaid
flowchart TD
    ECM["Калибровка ЭБУ NTE200:<br/>топл. код 0WH95 + отключённые защиты перегрева<br/>+ обороты 2100–2225 + задержки derate"] --> COMB["Иная карта впрыска<br/>+ нет снижения мощности при перегреве"]
...
- Банковая асимметрия: банк B горячее банка A на **~60 °C** — признак межбанкового дисбаланса (наддув/возврат/группировка дефектных форсунок).
...
Источник: [[Анализ нагрузок QSK50]]. Event 635 (перегрузка) — 26 677 событий, 46/47 машин (интерпретация — в Части II с учётом 10/10/20). Event 89 (RPM mismatch) — требует подтверждения.
...
|---|---|---|---|
| Калибровка ЭБУ (топл. код 0WH95 + отключённые защиты + обороты) | ~15 параметров ↓защиты; EGT 567 °C; FC611 ×46; асимметрия банков; письмо Cummins подтверждает | — | 🔴 Коренная |
...
> На NTE200 относительно 730E: **иной топливный код 0WH95** (vs FC0BU52 = иная карта впрыска); отключены C_EPD_OT_RPM_Drt_En и триггеры C_DO_01/07/08 (наддув/ОЖ/топливо); задержки derate 51 с (масло) и 43 с (ОЖ); max обороты 2100 (до 2225) vs 1990; droop 20% vs 0%; пороги горячего стопа ниже. (Параметр C_TIB_Fuel_Adjust_Upper_Limit 200% относится к учёту расхода топлива и в цепь не входит.) Итог — хронический перегрев и термоудар клапанов. См. [[Calibration_Compare_730E_vs_NTE200 (Calterm)]].
...
| NHL: отсутствие Stellite «нормально для rated» | NTE200 работает с повышенными оборотами и EGT 567 °C — это не rated; Stellite штатно в мировой практике | ❌ ложный аргумент (факт признан) |
...
### Немедленно 🔴
- [ ] Перекалибровать ЭБУ NTE200 под профиль 730E: топливный код/карта впрыска как у 730E, включить C_EPD_OT_RPM_Drt_En и триггеры C_DO_01/07/08, убрать задержки derate, снизить max обороты до 1990, droop 0% #действие/срочно
