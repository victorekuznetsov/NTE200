...
related:
  ...
  - "[[Calibration_Compare_730E_vs_NTE200 (Calterm)]]"
  - "[[Дерейты и тепловые защиты — 730E vs NTE200]]"
...
> [!abstract] Что задаёт калибровка ЭБУ NTE200 (и при чём тут TIB)
> Носитель отличий **впрыска** между платформами — **топливный код** (`0WH95` у NTE200 vs `FC0BU52` у 730E): он определяет карту давления рампы, фазы впрыска и дымовой ограничитель. Дополнительно на NTE200 **отключены тепловые защиты** (дерейт по T масла, триггеры перегрева) и **подняты обороты/droop** → перегрев не гасится сбросом нагрузки.
> **TIB (Trip Information Base)** — это подсистема **учёта рейса/расхода топлива** (телеметрия). Параметр `C_TIB_Fuel_Adjust_Upper_Limit` (100→200 %) относится к **учёту расхода (FCR) и на впрыск не влияет** — он исключён из причинной цепи (см. [[Calibration_Compare_730E_vs_NTE200 (Calterm)]]).
...
> EGT 567°C зафиксирован только на **NTE#82**. Для подтверждения системности нужен DML по всему парку NTE200 — это задача после перекалибровки ЭБУ (см. [[#Действия]]). Остальные три ключевых доказательства (топл. код 0WH95 + отключённые защиты, 0 отказов на 730E, отказ NTE#83 при 2776 м/ч) от одной машины не зависят.
